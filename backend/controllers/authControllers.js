...
exports.users = async(req,res)=>{
    try{
            ...
            product.rows.length > 0 ? res.status(200).json(product.rows) : res.status(404).json({message:"Users not found"})
        }catch(err){
            console.log(err);
        }
}
...
exports.register = async(req,res)=>{
    let {username, usersurname, birth_date, gender, phonenumber ,email, password} = req.body
            
    if(!username || !email || !password || !usersurname|| !birth_date || !gender || !phonenumber){
        ...
        try{
            ...
            if(hashedPassword){
                let result = await pool.query('insert into users (username, usersurname, birth_date, gender, phonenumber ,email, password) values ($1, $2, $3, $4, $5, $6, $7) returning * ', [username, usersurname, birth_date, gender, phonenumber ,email, hashedPassword])
